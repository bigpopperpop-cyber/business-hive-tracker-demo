...
const HiveForm: React.FC<HiveFormProps> = ({ onAdd, initialData, onCancel }) => {
  ...
  const summarizePollen = (data: any, hourIndex: number): string => {
    const types = [
  ...
  const fetchWeather = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      try {
        ...
        const hourIndex = entryDate.getHours();
        ...
        if (wData.hourly) {
          setWeather({
            temp: wData.hourly.temperature_2m[hourIndex],
            humidity: wData.hourly.relative_humidity_2m[hourIndex],
            pollenLevel: aData.hourly ? summarizePollen(aData.hourly, hourIndex) : 'Unknown',
            condition: `Code ${wData.hourly.weather_code[hourIndex]}`
          });
        }
      } catch (err) {
        console.error("Environment fetch failed", err);
        alert("Failed to fetch environment data for that date. Ensure the date is within range.");
      } finally {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <button
                  ...
                  title="Fetch environment data for the selected date and time"
                  ...
                  {isFetchingWeather ? (
                    ...
                    <div className="text-center w-full">
                      <div className="flex items-center justify-center space-x-1">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-tight">{weather.temp}°C / {weather.humidity}% Humidity</span>
                      </div>
                      <div className="flex items-center justify-center space-x-1 mt-0.5 px-2 overflow-hidden">
                    ...
                    <div className="flex flex-col items-center">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs font-bold uppercase">Fetch for Date</span>
                      </div>
                      <span className="text-[8px] opacity-60 uppercase font-black">Temp, Humidity & Pollen</span>
                    </div>
                  )}
                </button>
              </div>
